...
- **G0 의도 확정**: N:N = 1인 vs 다수 **AI**(프리포올, 이 데모 잔류) vs **네트워크로 연결된** 다수 인간
  (매치메이킹·서버 권위 필요 → 별도 멀티앱, ADR-005). **같은 기기에서 턴을 교대하는 로컬 다수 인간(hot-seat)은
  네트워크·서버가 전혀 없어 이 구분과 무관** — 별도로 이 데모에 잔류(Local PvP, 아래 항목).
- **G1**(잔류 시): 엔진 팀 일반화 — `USERS(2)`→`TEAMS[](N)`, `#rival`→others(), count/score/infect/win N팀, 라운드로빈 턴, N 시드.

### PHASE H — Local PvP(로컬 대전, 동일 기기 hot-seat)  ✅ 완료
- 서버·통화·매치메이킹·치팅방지 없음(ADR-005 비적용 판단, addendum 참고) — 완전정보 게임이라 정보 은닉도 불필요.
- **Seats 씬**(신규): 스테이지 `teams` 수만큼 좌석, 좌석별 [사람|AI] 토글 + AI 좌석 난이도. 이름 입력 없음(고정 "Player N").
- **Play 씬 확장**: 좌석 컨트롤러(`CONTROLLER`/`AI_LEVEL`) 기반으로 턴 분기 일반화(기존 `HUMAN` 하드코딩 제거).
  사람↔사람 전환 시 패스 오버레이(탭 확인, 오터치 차단). Pause 중 좌석별 기권(→AI easy 대타) 지원.
- **Result 씬 확장**: 순위(칸수 내림차순) + 무승부(공동 1위) 렌더, 재대결/좌석변경/스테이지선택.
- 회전 UI·이름 입력·5인 이상·전적 영속 저장은 범위 밖(설계 검토 후 불필요 판단).

> 진행 순서 확정: E → N:N → Local PvP → 광고 슬롯(최후, 실통합은 광고계정·동의배너 필요라 대기).

## 별도 앱으로 이관 (ADR-005/006)

**네트워크** PvP 멀티플레이 · 스킨 상점/통화 · 서버 권위 치팅방지(move-log 재시뮬). 이 데모 밖.
(로컬 hot-seat 대전은 이관 대상 아님 — PHASE H, ADR-005 addendum 참고)
